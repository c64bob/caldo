...
func TestDiscoverTaskCollections_GeneratesUniqueIDsForDuplicateBasenames(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/xml; charset=utf-8")
		if r.Method != "PROPFIND" {
			t.Fatalf("expected PROPFIND, got %s", r.Method)
...
<multistatus xmlns="DAV:" xmlns:c="urn:ietf:params:xml:ns:caldav">
  <response>
    <href>/dav/calendars/alice/team/tasks/</href>
    <propstat>
      <prop>
        <displayname>Team Tasks</displayname>
        <resourcetype><collection/><c:calendar/></resourcetype>
        <c:supported-calendar-component-set><c:comp name="VTODO"/></c:supported-calendar-component-set>
      </prop>
      <status>HTTP/1.1 200 OK</status>
  ...
  <response>
    <href>/dav/calendars/alice/personal/tasks/</href>
    <propstat>
      <prop>
  ...
  <response>
    <href>/dav/calendars/alice/project/tasks/</href>
    <propstat>
      <prop>
        <displayname>Project Tasks</displayname>
        <resourcetype><collection/><c:calendar/></resourcetype>
        <c:supported-calendar-component-set><c:comp name="VTODO"/></c:supported-calendar-component-set>
      </prop>
      <status>HTTP/1.1 200 OK</status>
	...
	client := NewClient()
	result, err := client.DiscoverTaskCollections(context.Background(), srv.URL, "alice", "pw", "")
	if err != nil {
		t.Fatalf("discover: %v", err)
	}
	if len(result.Collections) != 3 {
		t.Fatalf("expected 3 VTODO collections, got %d", len(result.Collections))
	}
	got := []string{result.Collections[0].ID, result.Collections[1].ID, result.Collections[2].ID}
	want := []string{"tasks", "tasks-2", "tasks-3"}
	for i := range want {
		if got[i] != want[i] {
			t.Fatalf("expected id[%d]=%q, got %q (all=%v)", i, want[i], got[i], got)
		}
	}
}
