...
func SavedFilterTasks(deps savedFilterDependencies) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := view.BaseLayoutWithTopbarAction(filter.Name, view.TaskListDisplayControls(display), view.ConfigurableTaskListPage(filter.Name, "Keine Aufgaben für diesen Filter.", groups)).Render(r.Context(), w); err != nil {
