...
func TestCompileFilter_LeafOperators(t *testing.T) {
	...
	tests := []struct {
		...
		{"completed-true", FilterNode{Operator: TokenCompleted, Value: "true"}, "(sync_status = 'completed' AND completed_at IS NOT NULL)", nil},
		{"completed-false", FilterNode{Operator: TokenCompleted, Value: "false"}, "(sync_status != 'completed' AND completed_at IS NULL)", nil},
