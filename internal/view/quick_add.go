...
func quickAddPreviewContent(draft parser.QuickAddDraft) templ.Component {
	return templ.ComponentFunc(func(_ context.Context, w io.Writer) error {
		...
		_, err := fmt.Fprint(w, `<section class="rounded border border-slate-300 p-4 dark:border-slate-700"><h3 class="font-medium">Vorschau</h3><ul class="mt-2 text-sm space-y-1"><li><strong>Titel:</strong> `+html.EscapeString(draft.Title)+`</li><li><strong>Projekt:</strong> `+html.EscapeString(draft.Project)+`</li><li><strong>Labels:</strong> `+labels+`</li><li><strong>Datum:</strong> `+html.EscapeString(draft.Due)+`</li><li><strong>Wiederholung:</strong> `+html.EscapeString(draft.Recurrence)+`</li><li><strong>Priorität:</strong> `+html.EscapeString(draft.Priority)+`</li></ul><form method="post" action="/tasks" class="mt-3"><input type="hidden" name="title" value="`+html.EscapeString(draft.Title)+`"/><input type="hidden" name="project_id" value="`+html.EscapeString(draft.ProjectID)+`"/><button type="submit" class="rounded bg-slate-900 px-3 py-2 text-white dark:bg-slate-100 dark:text-slate-900">Speichern</button></form></section></section>`)
		return err
	})
}
