...
func TestSettingsPageContentRendersCalDAVCalendarSyncUIAndSecuritySettings(t *testing.T) {
	...
	component := SettingsPageContent(SettingsPageView{
		Settings: db.AppSettings{
			...
			UILanguage:          "de",
			DarkMode:            "system",
			TaskNoteDisplay:     TaskNoteDisplayTwoLines,
	...
	for _, want := range []string{
		...
		`action="/sync/manual" data-sync-request hx-post="/sync/manual" hx-target="#sync-status" hx-swap="innerHTML"`,
		...
		`name="task_note_display"`,
		`Aufgabennotizen in Listen`,
		`value="none"`,
		`Nein`,
		`value="full"`,
		`Ja - vollständig`,
		`value="first_line"`,
		`Ja - erste Zeile`,
		`value="first_two_lines" selected`,
		`Ja - erste 2 Zeilen`,
		`name="ui_language"`,
		`name="dark_mode"`,
